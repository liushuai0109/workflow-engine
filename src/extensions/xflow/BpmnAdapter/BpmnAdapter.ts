/**
 * BPMN 适配器
 * 用于在 XPMN 格式（无前缀）和 BPMN 格式（带 bpmn: 或 xflow: 前缀）之间进行转换
 */
...
// 反向映射：从 BPMN 元素名（带前缀）映射到 XPMN 元素名
...
export function convertFromXPMNToBPMN(xml: string): string {
  try {
    ...
    console.error('Error converting from XPMN to BPMN:', error)
    throw error
  }
}
...
export function convertFromBPMNToXPMN(xml: string): string {
  try {
    ...
    console.error('Error converting from BPMN to XPMN:', error)
    throw error
  }
}

/**
 * 转换 definitions 元素从 XPMN 到 BPMN
...
/**
 * 转换 definitions 元素从 BPMN 到 XPMN
...
/**
 * 转换 XPMNDiagram 元素从 XPMN 到 BPMN（将 xpmn 恢复为 bpmn）
 */
...
function convertBPMNDiagramToXPMN(element: Element, doc: Document): Element {
  ...
  const tagName = localName.replace(/BPMN/g, 'XPMN').replace(/bpmn/g, 'xpmn')
