...
#### 问题 3：功能扩展受限
**现象**：需要更强大的 AI 能力支持工作流生成和智能推荐，但 Gemini 能力有限
**影响**：限制平台智能化能力和工作流生成质量
**证据**：复杂流程图生成质量不稳定，用户需要大量手动调整
...
3. **成本优化**：利用 Claude 的 Prompt Caching 功能降低 API 调用成本
...
### 删除规范
- **user-lifecycle**：删除 AI Agent 对话和智能推荐需求（范围外）
...
### 时间估算
- **预计总时间**：2-3 周（14-18 个工作日）
- **详细任务**：见 `tasks.md`（33 个任务，4 个阶段）
...
| 功能扩展受限 | 智能化能力受限 | 利用 Claude 更强能力 |
| 成本优化缺失 | 成本线性增长 | 集成 Prompt Caching |

### 范围总结
- **包含**：Claude API 集成、Function Calling、流式响应、对话管理、Prompt Caching、提示词优化
- **不包含**：多模型支持、本地部署、多媒体能力、后端服务、用户运营 AI Agent
...
- **总时长**：2-3 周（14-18 天）
- **阶段数**：4 个阶段，33 个任务
- **关键里程碑**：Phase 1（核心服务）→ Phase 2（工作流优化）→ Phase 3（对话管理）→ Phase 4（测试发布）
