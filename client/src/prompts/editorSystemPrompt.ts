...
⚠️ **视觉质量保证** - 以下规则确保流程图清晰、专业、无遮挡：

1. **连线避开节点**:
   - 回流线（从后往前）必须绕开中间的所有节点
   - 使用足够的水平/垂直偏移，在节点上方或下方绕行
   - 不允许连线覆盖任何节点区域

2. **连接点在边缘**:
   - 连线起点和终点应在节点的边缘（上下左右边界）
   - 不要连接到节点的中心点
   - 垂直流动：连到上边缘或下边缘
   - 水平流动：连到左边缘或右边缘
...
3. **文字标签不遮挡**:
   ...
   - 连线标签（如条件文字）不要与节点重叠
   - 连线标签之间也要避免重叠
   - 标签位置建议：在连线旁边偏移 10-20 像素

4. **回流线的绘制策略**:
   - 向上回流：先向上移动到所有节点上方（y 坐标减少），再水平移动，最后向下连到目标
   - 向下回流：先向下移动到所有节点下方（y 坐标增加），再水平移动，最后向上连到目标
   - 保证回流路径与主流程区域有足够间距（至少 50px）
...
- 审批节点 y=400
- 填写节点 y=250
- 回流线路径：从审批节点 → 向右偏移 → 向上移到 y=150（所有节点上方）→ 向左移到填写节点上方 → 向下连到填写节点顶部
\`\`\`
...
7. **⚠️ 布局质量**: 严格遵守上述"重要布局改进规则"，确保连线不覆盖节点、连接点在边缘、文字无遮挡
